...
namespace KanbanLight.Controllers
{
	...
	public class AccountController : Controller
	{
		...
		// Die Authorize-Aktion ist der Endpunkt, der aufgerufen wird, wenn Sie auf eine
		// geschützte Web API zugreifen. Wenn der Benutzer nicht angemeldet ist, wird er umgeleitet an 
		// die Login-Seite. Nach einer erfolgreichen Anmeldung können Sie eine Web API aufrufen.
		[HttpGet]
			...
			if (ModelState.IsValid)
			{
				...
				if (result.Succeeded)
				{
					...
					if (result.Succeeded)
					{
						...
						// string code = await UserManager.GenerateEmailConfirmationTokenAsync(user.Id);
						// var callbackUrl = Url.Action("ConfirmEmail", "Account", new { userId = user.Id, code = code }, protocol: Request.Url.Scheme);
						// SendEmail(user.Email, callbackUrl, "Konto bestätigen", "Bitte bestätigen Sie Ihr Konto, indem Sie auf diesen Link klicken.");
